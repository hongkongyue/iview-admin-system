<template>
    <div>
        <Row class="margin-bottom-10 background-color-white exhibition" style="font-size:20px;font-weight:bold">   
            {{taskCode}} / 外拍图片详情  <Button style="margin-top:10px;margin-right:20px;float:right" type="primary" @click="submit">提交</Button>
        </Row>
        <!-- 数据列表 -->
    ...
    export default {
                       ...
                       minWidth: 260,
                        render: (h, params) => {
                                  if ((this.page - 1) *this.pageSize + params.index == this.red_packet_table_row_index) {
                                    ...
                                    props: {
                                        width:150,
                                        placeholder:params.row.originalPhotoPath
                                    },
                                    style: {
                                            minWidth:'100px'
                                        },
                                    // on: {
                                    //     "on-change": (event) => {
                                    //         params.row.originalPhotoPath= event.target.value
                                    //         this.originalPhotoPath      = event.target.value
                                    //     }
                                    // },
                                    on: {
                                        "on-change": (event) => {
                                            ...
                                            if(event.target.value.length>200){
                                                this.$Message.warning('最大的输入长度为200字节')
                                                 params.row.originalPhotoPath= params.row.originalPhotoPath.substr(0,200)
                                            }
                                ...
                                return h('span', params.row.originalPhotoPath)
                            }
                        }
                    },
                    {
                                        ...
                                        click: () => {
                                               this.red_packet_table_row_index = -1;
                                                let data={};
                                                    ...
                                                    data.originalPhotoPath= params.row.originalPhotoPath
                                                     if(!data.originalPhotoPath)return this.$Message.error('原外拍图片路径不能为空！')
                                            this.request('productlaunch_saveDevelopOriginalPhotoPath',{
                                                data:data
                                            },true).then(res=>{
                                                if(res.code == 1){
        ...
        methods: {
            ...
            getData() {
                    ...
                    this.request('productlaunch_getDevelopOutdoorPhotoDetail', {data:data}, true).then((res) => {
                        if (res.code == 1) {
                             this.list2=[]
                             this.list2.push({
                             ...
                             this.originalPhotoPath= res.data.originalPhotoPath
                ...
                submit(){
                          ...
                          let data={};
                                ...
                                if(!data.originalPhotoPath)return this.$Message.error('原外拍图片路径不能为空！')
                                this.request('productlaunch_submitDevelopOutdoorPhoto',{
                                                data:data
                                },true).then(res=>{
                                                if(res.code == 1){
                                                    this.$Message.success(res.msg);
