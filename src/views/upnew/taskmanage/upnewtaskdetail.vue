<template>
    <div>
        <Row class="margin-bottom-10 background-color-white exhibition" style="font-size:20px;font-weight:bold">   
            {{taskCode}} / 上新管理 <Button style="margin-top:10px;margin-right:20px;float:right" type="primary" @click="submit">提交</Button>
        </Row>
                                        ...
                                        "on-change": (event) => {
                                            ...
                                            if(event.target.value.length>200){
                                                this.$Message.warning('最大的输入长度为200字节')
                                                 params.row.productLaunchPath= params.row.productLaunchPath.substr(0,200)
                                            }
                                ...
                                h('Button', {
                                 ...
                                 h('Button', {
                                    ...
                                    on: {
                                        click: () => {
                                                      this.red_packet_table_row_index = -1;
                                                      if((params.row.productLaunchPath).trim()==''||!params.row.productLaunchPath) return this.$Message.error('上新链接不能为空')
            ...
            //初始化第一个列表数据
             getData() {
                    ...
                    let data                     =  {}
                        ...
                        data.productLaunchStatus =  this.formSearch.productLaunchStatus;
                            ...
                            let data={};
                                ...
                                if(this.list.length>0){
                                    for(let i=0,len=this.list.length;i<len;i++){
                                        if(this.list[i].productLaunchStatus==0){
                                            return this.$Message.warning('请先完成任务，再提交！')
                                        }else if(this.list[i].finishStillLifePath == null){
                                            return this.$Message.warning('完成静物图片路径为空，不能提交！')
                                        }
                                    }
                                }
